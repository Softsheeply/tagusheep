...
/**
 * Collect every labeled maker number of one kind (RN or CA).
 *
 * @param {string} text
 * @param {RegExp} pattern
 * @returns {string[]}
 */
function collectLabeledNumbers(text, pattern) {
  const values = [];
  const seen = new Set();
  const flags = pattern.flags.includes("g") ? pattern.flags : `${pattern.flags}g`;
  const global = new RegExp(pattern.source, flags);
  let match;
  while ((match = global.exec(String(text || "")))) {
...
/**
 * US RN (or WPL) maker registration digits. CA numbers are a separate field.
 *
 * @param {string} text
 * @returns {string | null}
 */
...
/**
 * Every labeled RN on the tag.
 *
 * @param {string} text
 * @returns {string[]}
 */
export function extractRnNumbers(text) {
  return collectLabeledNumbers(text, /\bRN\s*[#:\-]?\s*(\d{3,8})\b/gi);
}

/**
 * Every labeled CA number on the tag.
 *
 * @param {string} text
 * @returns {string[]}
 */
export function extractCaNumbers(text) {
  return collectLabeledNumbers(text, /\bCA\s*[#:\-]?\s*(\d{3,8})\b/gi);
}

/**
 * Canadian CA identification number (five digits on care labels).
 * @param {string} text
 * @returns {string | null}
 */
export function extractCa(text) {
  return firstMatch(text, [/\bCA\s*[#:\-]?\s*(\d{5})\b/i]);
}
 ...
 * @returns {string | null}
 */
export function extractWpl(text) {
  return firstMatch(text, [/\bWPL\s*[#:\-]?\s*(\d{3,8})\b/i]);
}

/**
 * Which maker-ID label matched these digits in the text (RN preferred over CA).
 *
 * @param {string} text
 * @param {string | null | undefined} digits
 * @returns {"RN" | "CA" | null}
 */
export function makerIdKind(text, digits) {
  if (!digits) return null;
  const escaped = String(digits).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  if (new RegExp(`\\bRN\\s*[#:\\-]?\\s*${escaped}\\b`, "i").test(String(text || ""))) return "RN";
  if (new RegExp(`\\bCA\\s*[#:\\-]?\\s*${escaped}\\b`, "i").test(String(text || ""))) return "CA";
  return null;
}
